 ...
 * ENSG / IGN 
 * by Ahmad AUDI 
 * Génération d'une fractale de Mandelbrot.
 */
...
#pragma omp for private(j, pima, x, y) schedule(static)
    for (i = 0; i < nlines; i++) {
      y = ymin + yinc * rank * nlines + yinc * i;
      pima = &ima[i*w];
      x = xmin;
      for (j = 0; j < w; j++) {
  ...
  if( rank == MASTER ) {
    ...
    for(p = 1 ; p < nproc ; p++) {
      fprintf( stderr, "Preparing to receive data\n");
      MPI_Probe(MPI_ANY_SOURCE, RESULT, MPI_COMM_WORLD, &status);
      source = status.MPI_SOURCE;
      if(source != MASTER) {
	MPI_Recv(&ima[source * nlines * w],
		 nlines * w,
		 MPI_CHAR,
		 source,
		 RESULT,
    ...
    fprintf( stderr, "(%d) Preparing to send data\n", rank);
    MPI_Send(ima, w * nlines, MPI_CHAR, MASTER, RESULT, MPI_COMM_WORLD);
  }
